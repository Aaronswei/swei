...
namespace ml {
    ...
    int kd_insert(struct kdtree *tree, const double *pos, void *data)
    {
        ...
        if(tree->rect == 0){
            ...
            hyperrect_entend(tree->rect, pos);
        }
    ...
    static int find_nearest_n(struct kdnode *node, const double *pos, double range, int num, struct rheap *heap, int dim)
    {
        ...
        /*if the photon is close enough, add it th the result heap*/
        dist_sq = 0;
        for(i = 0; i < dim; i++){
            dist_sq += SQ(node->pos[i] - pos[i]);
        }
    ...
    static void kd_nearest_i(struct kdnode *node, const double *pos, struct kdnode **result, double *result_dist_sq, struct kdhyperrect* rect)
    {
        ...
        if(farther_subtree){
            ...
            *farther_hyperrect_coord = node->pos[dir];
             ...
             * point of the hyperrect and see if it's closer than our minimum 
             * distance in result_dist_sq. */
    ...
    struct kdres *kd_nearest(struct kdtree *kd, const double *pos)
    {
        ...
        hypperrect_free(rect);

        /*Store the result */
        if(result) {
            if(rlist_insert(rest->rlist, result, -1.0) == -1){
                kd_res_free(rset);
                return 0;
            }
            rset->size = 1;
    ...
    void kd_res_rewind(struct kdre *rset)
    {
        rset->riter = rset->rlist->next;
    }

    int kd_res_end(struct kdre *rset)
    {
        return rset->riter == 0;
    }
