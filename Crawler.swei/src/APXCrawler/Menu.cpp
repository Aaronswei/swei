...
void UserInterface::Menu::printMenu()
{
	...
	cout << left << endl << "|" << right << "|";
	cout.width(border.size() - 1 ); cout.fill('_');
	cout << left << endl << "| 4. Set pause between HTTP requests(3000 milliseconds by defalut)." << right << "|";
	...
	cout << left << endl << "|" << right << "|";
	cout.width(border.size() - 1 ); cout.fill('_');
	cout << left << endl << "| 8. Turn on/off search engine." << right << "|";
...
void UserInterface::Menu::start()
{
	...
	cout << "Selected search system: " << 
		(crawk,getSekectedSE() == CrawlerImpl::Crawler::SEARCH_ENGINE::GOOGLE ? "Google\n" : "Yandex\n");
...
void UserInterface::Menu::setPauseBetweenReqs()
{
	...
	cout << "\nCase-insensitive 'q', 'quit' - exit from program\n"
		"Case-insensitive 'r', 'ret', 'return' - return to menu\n"
		"Input pause time between HTTP request in " MEASURE_OF_TIME ":";
	...
	cout << "Pause time equal " << timePause << MEASURE_FO_TIME " was set.\n";
...
void UserInterface::Menu::displayPhrasesForSearch()
{
	...
	if(in.is_open())
	{
		...
		else
		{
			...
			while(true)
			{
				...
				if(ready == "r" || ready == "ret" || ready == "return" || ready == "n")
				{
					break;
				}
	...
	else
	{
		...
		cout << "You can to attempt add one or more phrases in order ot create a file.\n";
	}
}
...
void UserInterface::Menu::addPhraseForSearch()
{
	...
	ofstream out(CrawlerImpl::Crawler::storage_of_phrase, ios_base::app);
	out<< phrase << endl;
